...
def test_campaigns_hub_list_reflects_live_status_not_stale_disk(fixture_repo):
    """The actual reported production bug: resuming a campaign committed
    correctly to GitHub every time — the campaign's OWN detail page
    already showed "Running" correctly (fixed earlier) — but the hub
    list row for that same campaign kept showing "Paused" indefinitely,
    because _load_hub_rows built each row from get_campaign_cfg
    directly, which never consults GitHub at all. This is a fourth,
    separate consumer of campaign status that the earlier Settings /
    Schedule / detail-page-status fixes never reached."""
    # A lead not yet contacted at all — unambiguously still has work
    # pending, so an "active" status must compute to Running, not
    # Completed/Attention — isolating the one thing this test checks.
    fake_ws = {
        f"{FIXTURE_CAMPAIGN} Master Sheet": FakeWorksheet(
    ...
    with patch("gspread.authorize", return_value=type("C", (), {"open_by_key": lambda self, k: fake_spreadsheet})()), \
         ...
         patch("outreach.get_campaign", _fake_get_campaign_with_status("paused")):  # stale disk: paused
        at = AppTest.from_file(os.path.join(PAGES_DIR, "campaigns.py"))
        at.secrets.update(_dashboard_secrets())
        for k, v in _authed_session().items():
            at.session_state[k] = v
    ...
    assert list(at.exception) == []
    hub_body = " ".join(str(m.value) for m in at.markdown)
    assert "🟢 Running" in hub_body
    assert "⏸ Paused" not in hub_body
...
def test_sequences_tab_shows_locked_variants_for_real_campaign(fixture_repo):
    ...
    assert list(at.error) == []
    # The fixture campaign has 5 stages x 4 variants = 20 "Variant X" expanders
    variant_expanders = [e for e in at.expander if e.label.startswith("Variant ")]
    assert len(variant_expanders) == 20
    # Every EXISTING variant's own editor starts disabled (locked) — keyed
    # "subject_{stage}_{variant}" specifically. Filtering by key rather than
    # by label is what's needed here: the page also renders live, correctly
    # ENABLED subject inputs elsewhere (adding a new variant to an existing
    # stage, or — once a campaign isn't yet at the current stage cap, which
    # this fixture no longer is now that the cap is 10 follow-ups, not 4 —
    # adding a brand new follow-up stage). A label-based filter can't tell
    # those apart from the locked ones, since Streamlit renders every tab's
    # content regardless of which is visually selected.
    subject_inputs = [ti for ti in at.text_input if ti.key and ti.key.startswith("subject_")]
    assert len(subject_inputs) == 20
...
def test_sequences_tab_add_variant_maxed_out_for_real_campaign(fixture_repo):
    ...
    assert list(at.exception) == []
    info_texts = " ".join(i.value for i in at.info)
    # This test is specifically about ADDING A VARIANT being maxed out
    # (all 4 letters A-D already exist on every stage this fixture has)
    # — a completely separate concept from ADDING A STAGE being maxed
    # out, which used to coincidentally ALSO be true for this fixture
    # under the old 5-stage cap, but no longer is now that the cap is
    # 10 follow-ups: this fixture's 5 stages are no longer "all of
    # them", so asserting that text here would be checking something
    # this test was never actually about, and would just go stale again
    # the next time the fixture or the cap changes for unrelated reasons.
    assert "maximum" in info_texts.lower()
    assert "all 4 variants" in info_texts.lower()
...
def test_sequences_tab_delete_stage_reflects_live_structure_immediately_after_a_change(fixture_repo):
    ...
    fake_spreadsheet = FakeSpreadsheet(_campaigns_page_fake_ws())
    # Tracks the simulated deletion by an explicit flag, not a call
    # count — the page now legitimately makes more than one
    # list_directory_files call per render (a page-level live overlay
    # for status/schedule correctness, plus the Sequences tab's own
    # independent, error-surfacing fetch), and every one of them must
    # see the same, consistent "current" structure within a render.
    state = {"deleted": False}
    ...
    with patch("gspread.authorize", return_value=type("C", (), {"open_by_key": lambda self, k: fake_spreadsheet})()), \
        ...
        # Simulate the deletion actually having happened between renders.
        state["deleted"] = True
...
def test_status_controls_reflects_a_newly_added_stage_over_stale_disk(fixture_repo):
    """The actual reported production bug: a campaign whose only stage
    (intro) was fully sent correctly showed Completed. A follow-up
    stage added 24+ hours later committed the new template files fine
    — the Sequences tab even showed them correctly, since it already
    read live — but Status never left Completed, because
    compute_campaign_status reads campaign_cfg["stages"], which
    get_campaign_cfg built from the same frozen local template listing
    every OTHER live read on this page exists specifically to avoid.
    Must show Running once the live template listing reveals the new
    stage, regardless of what the stale local checkout still says."""
    lead = {"Email": "a@abc.com", "Approval": "Yes",
            "IntroSentAt": "2026-09-01 09:00:00", "IntroVariant": "A"}
    fake_ws = {
        f"{FIXTURE_CAMPAIGN} Master Sheet": FakeWorksheet([lead]),
        f"{FIXTURE_CAMPAIGN} Response Sheet": FakeWorksheet([]),
        f"{FIXTURE_CAMPAIGN} Custom Log Sheet": FakeWorksheet([]),
        f"{FIXTURE_CAMPAIGN} Error Log": FakeWorksheet([]),
    }
    ...
    def fake_list_directory_files(self, path, ref="main"):
        # Live GitHub: intro AND the newly-added followup1 both exist.
        return [f"{stage}_A.txt" for stage in ["intro", "followup1"]]
    ...
    with patch("gspread.authorize", return_value=type("C", (), {"open_by_key": lambda self, k: fake_spreadsheet})()), \
         ...
         patch("github_client.GitHubClient.list_directory_files", fake_list_directory_files), \
         patch("github_client.GitHubClient.get_file_content", fake_get_file_content), \
         patch("outreach.get_campaign", _fake_get_campaign_with_status("active")):  # stale disk: intro only
        at = AppTest.from_file(os.path.join(PAGES_DIR, "campaigns.py"))
        at.secrets.update(_dashboard_secrets())
        for k, v in _authed_session().items():
            at.session_state[k] = v
    ...
    subheaders = [h.value for h in at.subheader]
    assert "🟢 Running" in subheaders
    assert "✅ Completed" not in subheaders
