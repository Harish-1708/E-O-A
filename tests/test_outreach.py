...
# ---------- imap_fetch_recent: one malformed message must never block the rest ----------

def _make_fake_imap_for_fetch(raw_messages_by_id):
    """raw_messages_by_id: {b'1': raw_bytes, b'2': raw_bytes, ...}. Builds
    a fake IMAP4_SSL that returns exactly these message IDs from search,
    and their raw bytes from fetch — enough to exercise imap_fetch_recent
    end-to-end without a real mailbox."""
    ids_line = b" ".join(raw_messages_by_id.keys())
    ...
    class FakeIMAP:
        ...
        def fetch(self, num, spec):
            return "OK", [(num, raw_messages_by_id[num])]
...
def _raw_message(date_header, subject="Hi", message_id="<x@mail.gmail.com>"):
    msg = MIMEText("body")
    msg["Subject"] = subject
    msg["From"] = "a@b.com"
...
def test_imap_fetch_recent_one_malformed_message_does_not_block_the_rest(monkeypatch):
    """The actual reported production bug: a single message this parser
    can't handle — for any reason, not only a date header — used to
    raise straight out of the fetch loop and abort the ENTIRE mailbox,
    silently hiding every message after the bad one, from every sender.
    A genuinely malformed message must cost only itself."""
    good_before = _raw_message("Mon, 21 Sep 2026 10:00:00 -0400", subject="good one",
                                message_id="<good1@mail.gmail.com>")
    good_after = _raw_message("Mon, 21 Sep 2026 12:00:00 -0400", subject="good two",
                               message_id="<good2@mail.gmail.com>")
    real_parse = outreach._parse_email_message

    def flaky_parse(raw):
        if raw == b"BROKEN":
            raise RuntimeError("boom — simulates any per-message parsing failure")
        return real_parse(raw)

    monkeypatch.setattr(outreach, "_parse_email_message", flaky_parse)

    fake_ids = {b"1": good_before, b"2": b"BROKEN", b"3": good_after}
    monkeypatch.setattr(outreach.imaplib, "IMAP4_SSL", _make_fake_imap_for_fetch(fake_ids))

    messages = outreach.imap_fetch_recent("a@gmail.com", "app-pass",
                                           since_dt=datetime(2026, 9, 1))

    subjects = {m["subject"] for m in messages}
    assert subjects == {"good one", "good two"}
...
def test_mark_responses_read_builds_correct_batch_update(monkeypatch):
    ...
    class FakeResponsesWs:
        def row_values(self, n):
            return list(outreach.RESPONSES_COLUMNS)
...
# ---------- content-based dedup safeguard (video / refunnel matching) ----------

def test_lead_content_key_extracts_tiktok_id_from_video_file():
    lead = {"Video File": "https://www.tiktok.com/@handle/video/7669622655800069407"}
    assert outreach._lead_content_key(lead) == "7669622655800069407"


def test_lead_content_key_extracts_tiktok_id_from_refunnel_link_when_video_file_blank():
    lead = {"Video File": "", "Refunnel Link": "https://cdn.refunnel.com/hls/tk_7669622655800069407_0/x.m3u8"}
    assert outreach._lead_content_key(lead) == "7669622655800069407"


def test_lead_content_key_falls_back_to_raw_lowercased_string_when_no_id_extractable():
    lead = {"Video File": "https://tiktok.com/t/AbCdEfG/", "Refunnel Link": ""}
    assert outreach._lead_content_key(lead) == "https://tiktok.com/t/abcdefg/"


def test_lead_content_key_none_when_neither_field_present():
    assert outreach._lead_content_key({}) is None
    assert outreach._lead_content_key({"Video File": "", "Refunnel Link": ""}) is None


def test_asana_build_existing_task_content_index_skips_the_call_when_fields_not_configured(monkeypatch):
    """A project that doesn't use Video File / Refunnel Link at all must
    pay zero extra cost — no API call at all, not even one."""
    def fail_if_called(method, url, headers=None, timeout=None, json=None):
        raise AssertionError("must not call Asana at all when neither field is configured")
    monkeypatch.setattr(outreach.requests, "request", fail_if_called)
    structure = {"custom_fields": {"Creator": {"gid": "f_creator", "type": "text", "options": {}}}}
    assert outreach.asana_build_existing_task_content_index("proj_1", structure, "fake-key") == {}


def test_asana_build_existing_task_content_index_builds_key_to_gid_map(monkeypatch):
    structure = {"custom_fields": {"Video File": {"gid": "f_video", "type": "text", "options": {}}}}

    def fake_request(method, url, headers=None, timeout=None, json=None):
        assert "/tasks?project=proj_1" in url
        return FakeAsanaResponse({"data": [
            {"gid": "task_A", "created_at": "2026-09-01T00:00:00Z",
             "custom_fields": [{"gid": "f_video", "display_value": "https://tiktok.com/@x/video/111111"}]},
            {"gid": "task_B", "created_at": "2026-09-02T00:00:00Z",
             "custom_fields": [{"gid": "f_video", "display_value": ""}]},
        ]})
    monkeypatch.setattr(outreach.requests, "request", fake_request)

    index = outreach.asana_build_existing_task_content_index("proj_1", structure, "fake-key")
    assert index == {"111111": "task_A"}


def test_asana_build_existing_task_content_index_paginates_through_every_page(monkeypatch):
    """A project can hold far more tasks than one page returns — every
    page must be checked, not just the first."""
    structure = {"custom_fields": {"Video File": {"gid": "f_video", "type": "text", "options": {}}}}
    calls = []
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if "offset=" not in url:
            return FakeAsanaResponse({
                "data": [{"gid": "task_A", "created_at": "2026-09-01T00:00:00Z",
                           "custom_fields": [{"gid": "f_video", "display_value": "/video/111111"}]}],
                "next_page": {"offset": "page2token"},
            })
        return FakeAsanaResponse({
            "data": [{"gid": "task_B", "created_at": "2026-09-02T00:00:00Z",
                       "custom_fields": [{"gid": "f_video", "display_value": "/video/222222"}]}],
        })
    monkeypatch.setattr(outreach.requests, "request", fake_request)

    index = outreach.asana_build_existing_task_content_index("proj_1", structure, "fake-key")
    assert index == {"111111": "task_A", "222222": "task_B"}
    assert any("page2token" in u for u in calls)


def test_asana_build_existing_task_content_index_oldest_task_wins_on_collision(monkeypatch):
    structure = {"custom_fields": {"Video File": {"gid": "f_video", "type": "text", "options": {}}}}

    def fake_request(method, url, headers=None, timeout=None, json=None):
        return FakeAsanaResponse({"data": [
            {"gid": "task_NEWER", "created_at": "2026-09-05T00:00:00Z",
             "custom_fields": [{"gid": "f_video", "display_value": "/video/111111"}]},
            {"gid": "task_OLDER", "created_at": "2026-09-01T00:00:00Z",
             "custom_fields": [{"gid": "f_video", "display_value": "/video/111111"}]},
        ]})
    monkeypatch.setattr(outreach.requests, "request", fake_request)

    index = outreach.asana_build_existing_task_content_index("proj_1", structure, "fake-key")
    assert index == {"111111": "task_OLDER"}


def test_sync_campaign_to_asana_reuses_existing_task_instead_of_creating_a_duplicate(monkeypatch):
    """End-to-end reproduction of the actual reported incident: a lead
    with no AsanaTaskGID recorded (whatever the reason — this is the
    exact scenario a lost/never-written GID produces) whose video
    already matches a task that exists in the project. Must reuse that
    task, not create another one."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com", AsanaTaskGID="",
                      **{"Video File": "https://www.tiktok.com/@mrzannabanana/video/7669622655800069407"})
    fake_sheets = FakeSheets([lead])
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if method == "GET" and "/projects/proj_1?" in url:
            return FakeAsanaResponse({"data": {
                "sections": [{"name": "Sourced", "gid": "sec_sourced"}],
                "custom_field_settings": [
                    {"custom_field": {"name": "Video File", "gid": "f_video", "resource_subtype": "text"}},
                ],
            }})
        if method == "GET" and "/tasks?project=" in url:
            return FakeAsanaResponse({"data": [
                {"gid": "existing_task_gid", "created_at": "2026-09-01T00:00:00Z",
                 "custom_fields": [{"gid": "f_video", "display_value": "/video/7669622655800069407"}]},
            ]})
        raise AssertionError(f"must not create a new task — unexpected call: {method} {url}")
    ...
    assert fake_sheets._leads[0]["AsanaTaskGID"] == "existing_task_gid"


def test_sync_campaign_to_asana_still_creates_when_no_content_match_exists(monkeypatch):
    """The safeguard must never block a GENUINELY new lead — one whose
    video doesn't match anything already in the project must still get
    a real new task created."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com", AsanaTaskGID="",
                      **{"Video File": "https://www.tiktok.com/@newcreator/video/999999999"})
    fake_sheets = FakeSheets([lead])
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if method == "GET" and "/tasks?project=" in url:
            return FakeAsanaResponse({"data": [
                {"gid": "unrelated_task", "created_at": "2026-09-01T00:00:00Z",
                 "custom_fields": [{"gid": "f_video", "display_value": "/video/111111"}]},
            ]})
    ...
    assert fake_sheets._leads[0]["AsanaTaskGID"] == "brand_new_gid"


# ---------- _call_with_asana_retries ----------

def test_call_with_asana_retries_recovers_from_a_transient_timeout(monkeypatch):
    """The actual reported production bug: a read timeout — ordinary
    network jitter across the ~150+ Asana calls one sync run makes —
    raises requests.exceptions.Timeout directly rather than returning an
    object with a .status_code, so the ORIGINAL wrapper's status-code
    check never even ran. A single transient timeout permanently failed
    that one lead for the entire run with zero retry, even though the
    exact same request succeeds immediately after."""
    ...
    def flaky(*a, **kw):
        attempts["count"] += 1
        if attempts["count"] < 2:
            raise outreach.requests.exceptions.ReadTimeout("simulated read timeout")
        return FakeAsanaResponse({"data": {}})

    result = outreach._call_with_asana_retries(flaky)
    assert result.status_code == 200
    assert attempts["count"] == 2


def test_call_with_asana_retries_recovers_from_a_connection_error(monkeypatch):
    monkeypatch.setattr(outreach.time, "sleep", lambda s: None)
    attempts = {"count": 0}
    ...
    def flaky(*a, **kw):
        attempts["count"] += 1
        if attempts["count"] < 3:
            raise outreach.requests.exceptions.ConnectionError("simulated connection reset")
        return FakeAsanaResponse({"data": {}})
...
def test_call_with_asana_retries_raises_after_persistent_timeout(monkeypatch):
    monkeypatch.setattr(outreach.time, "sleep", lambda s: None)

    def always_times_out(*a, **kw):
        raise outreach.requests.exceptions.ReadTimeout("simulated persistent timeout")

    with pytest.raises(outreach.requests.exceptions.ReadTimeout):
        outreach._call_with_asana_retries(always_times_out)


def test_call_with_asana_retries_timeout_uses_the_same_backoff_as_5xx(monkeypatch):
    """Network-level failures and HTTP 5xx failures should wait the same
    schedule — both are "worth retrying, nothing more is known"."""
    sleeps = []
    monkeypatch.setattr(outreach.time, "sleep", lambda s: sleeps.append(s))
    attempts = {"count": 0}

    def flaky(*a, **kw):
        attempts["count"] += 1
        if attempts["count"] < 3:
            raise outreach.requests.exceptions.ReadTimeout("simulated")
        return FakeAsanaResponse({"data": {}})

    outreach._call_with_asana_retries(flaky)
    assert sleeps == [outreach.ASANA_RETRY_BASE_DELAY_SECONDS * (2 ** 0),
                       outreach.ASANA_RETRY_BASE_DELAY_SECONDS * (2 ** 1)]


def test_call_with_asana_retries_a_real_404_is_never_retried_or_swallowed(monkeypatch):
    """The original guarantee must survive — a definitive HTTP error
    (403, 404, a real auth problem) must still be returned immediately,
    not retried and not confused with a network failure."""
    def fail_if_called(s):
        raise AssertionError("must not sleep/retry for a definitive 404")
    monkeypatch.setattr(outreach.time, "sleep", fail_if_called)
    result = outreach._call_with_asana_retries(lambda *a, **kw: FakeAsanaResponse({}, status_code=404))
...
def test_sync_campaign_to_tracker_sheet_explicit_override_can_still_move_a_frozen_row():
    """The actual bug found while investigating a reported production
    regression: the freeze was applied purely from the tracker row's
    OWN current_status, with no check of the lead's own
    ManualAsanaStage at all — meaning a row already at a manual-only
    status (Rights Secured / Declined) could never be moved again by
    ANY means, including a deliberate, explicit override, contradicting
    the documented design that an explicit override is never blocked.
    An explicit override correcting or advancing an already-finalized
    row must actually take effect."""
    lead = make_lead(Email="rocky@abc.com", ManualAsanaStage="Negotiating")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky",
    ...
    status_col = tracker_ws.row_values(1).index("Contact Status") + 1
    written_status = next((v for _r, c, v in tracker_ws.updated_cells if c == status_col), None)
    assert written_status == outreach.TRACKER_STATUS_NEGOTIATING


def test_sync_campaign_to_tracker_sheet_no_override_still_freezes_a_manual_status_row():
    """The original guarantee must still hold with no explicit override
    at all — an AUTOMATIC recomputation must never move a row out of a
    manual-only status on its own."""
    lead = make_lead(Email="rocky@abc.com", ReplyStatus="Replied")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
...
# ---------- fill-blanks-only once a lead reaches Negotiating+ ----------

def test_filter_custom_fields_to_blanks_only_drops_already_set_field():
    """The actual reported production bug: a human manually sets a
    negotiated Rights Expiration date directly in Asana, and the next
    scheduled sync silently overwrote it back to the Sheet's own, older
    value, every single run."""
    payload = {"rights_exp_gid": {"date": "2026-09-01"}, "product_gid": "DudeRobe"}
    blank_gids = {"product_gid"}  # only Product is currently blank on the live task
    filtered = outreach.filter_custom_fields_to_blanks_only(payload, blank_gids)
    assert filtered == {"product_gid": "DudeRobe"}
    assert "rights_exp_gid" not in filtered


def test_filter_custom_fields_to_blanks_only_keeps_everything_when_all_blank():
    payload = {"a": 1, "b": 2}
    assert outreach.filter_custom_fields_to_blanks_only(payload, {"a", "b"}) == payload


def test_filter_custom_fields_to_blanks_only_drops_everything_when_none_blank():
    payload = {"a": 1, "b": 2}
    assert outreach.filter_custom_fields_to_blanks_only(payload, set()) == {}


def test_asana_get_task_blank_custom_field_gids_uses_display_value(monkeypatch):
    """display_value is what's checked, since it's the one representation
    common to every custom field type (text, number, date, enum,
    multi_enum) — not a type-specific value that would need separate
    handling per field type."""
    def fake_request(method, url, headers=None, timeout=None, json=None):
        assert "custom_fields.gid,custom_fields.display_value" in url
        return FakeAsanaResponse({"data": {"custom_fields": [
            {"gid": "g1", "display_value": "2027-06-15"},
            {"gid": "g2", "display_value": ""},
            {"gid": "g3", "display_value": None},
        ]}})
    monkeypatch.setattr(outreach.requests, "request", fake_request)
    assert outreach.asana_get_task_blank_custom_field_gids("task123", "fake-key") == {"g2", "g3"}


def test_sync_campaign_to_asana_negotiating_task_never_overwrites_a_set_field(monkeypatch):
    """Full end-to-end reproduction: a task LIVE in Negotiating, with a
    human-set Rights Expiration, syncing a lead whose Sheet has an
    older Rights Duration value — the human's date must survive."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com", AsanaTaskGID="existing_gid_123",
                      **{"Rights Duration": "2026-09-01"})
    fake_sheets = FakeSheets([lead])
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if method == "GET" and "/projects/proj_1?" in url:
            return FakeAsanaResponse({"data": {
                "sections": [{"name": "Negotiating", "gid": "sec_negotiating"}],
                "custom_field_settings": [
                    {"custom_field": {"gid": "rights_exp_gid", "name": "Rights Duration",
                                       "resource_subtype": "date", "enum_options": []}},
                ],
            }})
        if method == "GET" and "/tasks/existing_gid_123?" in url and "opt_fields=memberships" in url:
            return FakeAsanaResponse({"data": {
                "memberships": [{"project": {"gid": "proj_1"}, "section": {"name": "Negotiating"}}],
            }})
        if method == "GET" and "/tasks/existing_gid_123?" in url and "display_value" in url:
            # Live truth: a human already set this field directly in Asana.
            return FakeAsanaResponse({"data": {"custom_fields": [
                {"gid": "rights_exp_gid", "display_value": "2027-06-15"},
            ]}})
        if method == "PUT" and "/tasks/existing_gid_123" in url:
            assert "rights_exp_gid" not in (json or {}).get("data", {}).get("custom_fields", {}), (
                "the human-set field must never appear in the update payload at all"
            )
            return FakeAsanaResponse({"data": {}})
        raise AssertionError(f"Unexpected call: {method} {url}")
...
def test_sync_campaign_to_asana_warns_when_stale_override_reverts_a_direct_asana_move(monkeypatch, capsys):
    """The exact reported production scenario, reproduced with the real
    lead's own data: a lead's ManualAsanaStage was set to "Negotiating"
    at some earlier point and never cleared. The task was then moved
    directly in Asana to "Rights Secured" — a real human decision made
    outside the app entirely. On the next sync, the explicit override
    correctly wins (by design — this is also what stops the
    multi-campaign regression bug), so the task moves BACK to
    Negotiating. That's correct behavior, but it was happening
    completely silently, with nothing anywhere explaining why a fresh
    Asana-side change didn't stick. This proves the warning fires with
    enough detail (both the live section it's leaving and the stale
    override causing it) for the user to actually diagnose it."""
    lead = make_lead(_row=2, LeadID="L1", Email="anthonyconnorfit@gmail.com",
                      AsanaTaskGID="1218598786820265", ManualAsanaStage="Negotiating")
    fake_sheets = FakeSheets([lead])
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if method == "GET" and "/tasks/1218598786820265?" in url and "opt_fields=memberships" in url:
            # Live truth: the user dragged this task to Rights Secured by hand.
            return FakeAsanaResponse({"data": {
                "memberships": [{"project": {"gid": "proj_1"}, "section": {"name": "Rights Secured"}}],
            }})
        if method == "GET" and "assignee" in url.lower():
            return FakeAsanaResponse({"data": {"assignee": None}})
        if method == "PUT" and "/tasks/1218598786820265" in url:
            return FakeAsanaResponse({"data": {}})
        if method == "POST" and "/addTask" in url:
            return FakeAsanaResponse({"data": {}})
        raise AssertionError(f"Unexpected call: {method} {url}")
    ...
    assert result["errors"] == []
    out = capsys.readouterr().out
    assert "anthonyconnorfit@gmail.com" in out
    assert "Rights Secured" in out  # the live section it's being moved away from
    assert "Negotiating" in out     # the stale override causing it
    assert "Manage a lead" in out   # actionable — tells the user how to fix it


def test_sync_campaign_to_asana_no_warning_when_override_matches_live_section(monkeypatch, capsys):
    """The warning must only fire when the override is actually
    REDIRECTING the task away from where it currently sits — not on
    every sync of every lead that happens to have an override set."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com",
                      AsanaTaskGID="existing_gid_1", ManualAsanaStage="Rights Secured")
    fake_sheets = FakeSheets([lead])
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if method == "GET" and "/projects/proj_1?" in url:
            return FakeAsanaResponse({"data": {
                "sections": [{"name": "Rights Secured", "gid": "sec_rights"}],
                "custom_field_settings": [],
            }})
        if method == "GET" and "/tasks/existing_gid_1?" in url and "opt_fields=memberships" in url:
            return FakeAsanaResponse({"data": {
                "memberships": [{"project": {"gid": "proj_1"}, "section": {"name": "Rights Secured"}}],
            }})
        if method == "PUT" and "/tasks/existing_gid_1" in url:
            return FakeAsanaResponse({"data": {}})
        raise AssertionError(f"Unexpected call: {method} {url}")
    ...
    out = capsys.readouterr().out
    assert "rocky@abc.com" not in out


def test_sync_campaign_to_asana_no_warning_when_no_override_set(monkeypatch, capsys):
    """No ManualAsanaStage at all — an ordinary automatic sync — must
    never trigger this warning, regardless of what stage it computes."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com",
                      AsanaTaskGID="existing_gid_1", ReplyStatus="Replied")
    fake_sheets = FakeSheets([lead])
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if method == "GET" and "/projects/proj_1?" in url:
            return FakeAsanaResponse({"data": {
                "sections": [{"name": "Outreach Sent", "gid": "sec_os"}, {"name": "Negotiating", "gid": "sec_neg"}],
                "custom_field_settings": [],
            }})
        if method == "GET" and "/tasks/existing_gid_1?" in url and "opt_fields=memberships" in url:
            return FakeAsanaResponse({"data": {
...
def test_sync_campaign_to_asana_earlier_stage_never_makes_the_extra_blank_check_call(monkeypatch):
    """The efficiency guarantee: a lead NOT yet in Negotiating+ must
    never trigger the extra blank-field lookup call at all — most leads
    on any given run are earlier than that, and this must not add an
    API call for every one of them."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com", AsanaTaskGID="existing_gid_123")
    fake_sheets = FakeSheets([lead])
    fake_request, calls = _make_fake_asana_request(task_sections={"existing_gid_123": "Outreach Sent"})
    monkeypatch.setattr(outreach.requests, "request", fake_request)
    ...
    blank_check_calls = [c for c in calls if c[0] == "GET" and "display_value" in c[1]]
    assert blank_check_calls == []
...
def test_sync_campaign_to_asana_one_lead_error_does_not_block_others(monkeypatch):
    ...
    # This failure is PERSISTENT (every call to this URL raises), so the new
    # network-error retry logic correctly retries ASANA_MAX_RETRIES times
    # before giving up — real exponential backoff would make this test
    # take ~14s for no benefit; mock the sleep, not the retry behavior.
    monkeypatch.setattr(outreach.time, "sleep", lambda s: None)
...
def test_auto_send_all_skips_non_running_campaigns(monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "nothing to do" in out
    # The actual reported production issue: this used to be completely
    # silent about WHY a campaign was excluded, which looked exactly
    # like "auto-send only ever processes one hardcoded campaign" —
    # every other campaign was in fact considered, just correctly
    # excluded by its own status, with zero visibility into that.
    assert "Foo: skipped (status is 'paused', not active)." in out
    assert "Bar: skipped (status is 'draft', not active)." in out


def test_auto_send_all_mixed_statuses_only_explains_the_skipped_ones(monkeypatch, capsys):
    """The transparency fix must not spam anything for campaigns that
    WERE processed — only the ones excluded need an explanation."""
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {})
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["ActiveCamp", "PausedCamp"])
    monkeypatch.setattr(outreach, "get_campaign",
                         lambda name, **kw: {"status": "active" if name == "ActiveCamp" else "paused",
                                              "stages": []})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: object())

    outreach.cmd_auto_send_all(argparse.Namespace())

    out = capsys.readouterr().out
    assert "PausedCamp: skipped (status is 'paused', not active)." in out
    assert "ActiveCamp: skipped" not in out
...
def _make_sheets_connector_with_master_ws(master_ws):
    """Builds a real SheetsConnector instance around a fake worksheet,
    bypassing __init__ (which needs a live gspread client and
    spreadsheet connection) — this is what actually lets
    update_lead_fields / update_lead_statuses be tested directly against
    the REAL implementation, rather than only ever through FakeSheets'
    own, separate, hand-written stand-in for them. Confirmed by direct
    grep that NEITHER of these two real methods had ANY direct test
    coverage before this — every existing test exercised FakeSheets'
    own fake versions instead, which is exactly how a real, severe bug
    in the ACTUAL implementation went uncaught."""
    connector = object.__new__(outreach.SheetsConnector)
    connector.master_ws = master_ws
    connector._gspread = outreach.gspread  # rowcol_to_a1 is a pure utility, safe to use directly
    return connector


class FakeWs:
    def __init__(self, header=None, col_count=None):
        self._header = header if header is not None else []
        self.appended_rows = []
        self.updated_cells = []  # [(row, col, value), ...]
        self.resize_calls = []
        # Mirrors a real gspread Worksheet's fixed grid width — defaults
        # to exactly the starting header's length (the realistic "this
        # tab has never been widened" case) unless a test deliberately
        # wants a wider or narrower starting grid.
        self.col_count = col_count if col_count is not None else len(self._header)
    ...
    def batch_update(self, updates):
        """Mirrors real gspread: each entry is {"range": "A1"-style,
        "values": [[value]]}. Parses the A1 range back to (row, col) so
        callers can assert against the same (row, col, value) shape
        update_cell already produces, regardless of which of the two a
        given piece of code happens to use."""
        import re as _re
        for u in updates:
            ...
            self.updated_cells.append((int(row_str), col, u["values"][0][0]))
...
class _GridLimitResponse:
    """A minimal fake matching what gspread.exceptions.APIError expects
    from a requests Response — just enough for StrictFakeWs below to
    raise something realistic."""
    def __init__(self, col_count):
        self.status_code = 400
        self._col_count = col_count

    def json(self):
        return {"error": {"code": 400, "message": f"Range exceeds grid limits. Max columns: {self._col_count}"}}


class StrictFakeWs(FakeWs):
    """Unlike the lenient FakeWs used everywhere else, this one actually
    ENFORCES the grid's column limit the way Google Sheets' real API
    does — update_cell raises if the target column exceeds col_count,
    exactly like the real, reported [400] "Range exceeds grid limits"
    error. Used specifically to prove the resize genuinely happens
    BEFORE the write that needs it, not just that resize() gets called
    at some point regardless of whether it was actually necessary."""
    def update_cell(self, row, col, value):
        if col > self.col_count:
            raise outreach.gspread.exceptions.APIError(_GridLimitResponse(self.col_count))
        super().update_cell(row, col, value)


def test_get_or_create_ws_resizes_grid_before_widening_the_exact_reported_case(monkeypatch):
    """The actual reported production bug, reproduced with the EXACT
    numbers involved: a Master Sheet tab whose grid was sized to 39
    columns (created before MASTER_COLUMNS was extended from 34 to 46
    columns for the follow-up-count increase), now needing to grow past
    that — the real Google Sheets API rejects a write beyond the
    current grid width outright with a 400 error, regardless of how
    correct the header content itself is. ensure_master_header_includes
    already had this exact protection for CUSTOM columns; this function
    — used for every SheetsConnector construction, so hit on every
    single command against an older Sheet — never did."""
    # 39 columns already there, matching the literal reported grid size.
    existing_header = list(outreach.MASTER_COLUMNS[:39])
    existing_ws = StrictFakeWs(header=list(existing_header), col_count=39)
    spreadsheet = FakeSpreadsheet(existing={"Kelson_Creators_Licensing_17SEP Master Sheet": existing_ws})

    outreach._get_or_create_ws(spreadsheet, FakeGspreadModule,
                                "Kelson_Creators_Licensing_17SEP Master Sheet", outreach.MASTER_COLUMNS)

    assert existing_ws.row_values(1) == outreach.MASTER_COLUMNS
    assert existing_ws.col_count >= len(outreach.MASTER_COLUMNS)
    assert len(existing_ws.resize_calls) == 1


def test_get_or_create_ws_does_not_resize_when_grid_already_wide_enough():
    """The resize must be conditional — a tab whose grid is already big
    enough (e.g. from an earlier custom-column widening) must not get a
    pointless extra resize call on every single connection."""
    existing_header = list(outreach.MASTER_COLUMNS[:-1])
    existing_ws = StrictFakeWs(header=list(existing_header), col_count=len(outreach.MASTER_COLUMNS) + 20)
    spreadsheet = FakeSpreadsheet(existing={"Foo Master Sheet": existing_ws})

    outreach._get_or_create_ws(spreadsheet, FakeGspreadModule, "Foo Master Sheet", outreach.MASTER_COLUMNS)

    assert existing_ws.resize_calls == []


# ---------- update_lead_fields / update_lead_statuses: write by NAME, not position ----------

def test_update_lead_fields_writes_by_name_when_sheet_header_order_differs_from_master_columns():
    """The actual reported production bug, confirmed directly against a
    real Asana project: this real sheet's header has AsanaTaskGID at a
    DIFFERENT position than MASTER_COLUMNS' own Python list position —
    exactly what happens for every existing campaign the moment
    MASTER_COLUMNS gains a new entry INSERTED before an existing one
    (extending follow-ups from 4 to 10 inserted 12 new columns before
    AsanaTaskGID). _get_or_create_ws deliberately APPENDS any missing
    column to the end of whatever header already exists — it never
    reorders existing columns to match MASTER_COLUMNS' internal order —
    so the two orders diverging is the NORMAL, expected state for any
    sheet that has been incrementally widened over time, not an edge
    case. Writing by position previously sent the new Asana task GID
    into a completely different, wrong column, so the sheet's REAL
    AsanaTaskGID cell stayed blank — meaning every following sync
    treated the lead as brand new and created another duplicate task,
    for literally every lead, every run."""
    # A header order that genuinely differs from MASTER_COLUMNS' own
    # order: AsanaTaskGID sits early here, with several custom columns
    # and even some MASTER_COLUMNS entries AFTER it — a completely
    # realistic shape for a sheet incrementally widened many times.
    header = ["LeadID", "Email", "AsanaTaskGID", "Ad Ready", "Client", "Status", "LastActionAt"]
    ws = FakeWs(header=list(header))
    connector = _make_sheets_connector_with_master_ws(ws)

    connector.update_lead_fields(2, {"AsanaTaskGID": "new_task_gid_123"})

    # Must land in AsanaTaskGID's REAL column (index 2, 0-based -> col 3),
    # never MASTER_COLUMNS' own Python list position for that name.
    assert ws.updated_cells == [(2, 3, "new_task_gid_123")]


def test_update_lead_fields_multiple_fields_each_written_to_their_real_column():
    header = ["LeadID", "Status", "Email", "AsanaTaskGID", "LastActionAt"]
    ws = FakeWs(header=list(header))
    connector = _make_sheets_connector_with_master_ws(ws)

    connector.update_lead_fields(5, {"AsanaTaskGID": "gid1", "Status": "Stopped - Manual"})

    written = {col: val for _row, col, val in ws.updated_cells}
    assert written[header.index("AsanaTaskGID") + 1] == "gid1"
    assert written[header.index("Status") + 1] == "Stopped - Manual"


def test_update_lead_fields_rejects_a_column_not_actually_on_this_sheet():
    """A column that's a real MASTER_COLUMNS name but genuinely isn't on
    THIS sheet's header yet (never widened for it) must still raise —
    silently writing to a wrong/nonexistent position would be worse
    than a clear error."""
    ws = FakeWs(header=["LeadID", "Email"])
    connector = _make_sheets_connector_with_master_ws(ws)
    with pytest.raises(ValueError):
        connector.update_lead_fields(2, {"AsanaTaskGID": "gid1"})


def test_update_lead_statuses_writes_by_name_when_header_order_differs():
    """Same fix, same reason, as update_lead_fields — Status and
    LastActionAt are ALSO after the follow-up-count insertion point in
    MASTER_COLUMNS' own list, so bulk status updates (e.g. the
    soft-remove-a-lead flow) were writing 'Removed' into a completely
    wrong column on every existing campaign's sheet — real, silent data
    corruption, not just a cosmetic sync issue."""
    header = ["LeadID", "Status", "Email", "LastActionAt", "AsanaTaskGID"]
    ws = FakeWs(header=list(header))
    connector = _make_sheets_connector_with_master_ws(ws)

    connector.update_lead_statuses({3: "Removed"})

    written = {col: val for _row, col, val in ws.updated_cells}
    assert written[header.index("Status") + 1] == "Removed"
    assert header.index("LastActionAt") + 1 in written  # timestamp stamped too


def _make_sheets_connector_with_responses_ws(responses_ws):
    """Same bypass-__init__ pattern as _make_sheets_connector_with_master_ws,
    for the Responses sheet's own methods."""
    connector = object.__new__(outreach.SheetsConnector)
    connector.responses_ws = responses_ws
    connector._gspread = outreach.gspread
    return connector


def test_get_logged_message_ids_writes_by_name_when_responses_header_order_differs():
    """The exact same bug class found on the Master sheet, confirmed to
    also exist here as a latent, not-yet-triggered risk: a Responses
    sheet incrementally widened over time can have a header order that
    genuinely differs from RESPONSES_COLUMNS' own Python list order,
    for the identical reason _get_or_create_ws appends missing columns
    to the end rather than reordering to match. Reading MessageID by a
    fixed list position rather than the sheet's own real header would
    silently read the wrong column's values as message IDs."""
    header = ["ResponseID", "MessageID", "SomeCustomColumn", "Subject"]
    ws = FakeWs(header=list(header))
    ws.col_values = lambda col: {2: ["MessageID", "abc123", "def456"]}.get(col, [])
    connector = _make_sheets_connector_with_responses_ws(ws)
    assert connector.get_logged_message_ids() == {"abc123", "def456"}


def test_get_logged_message_info_writes_by_name_when_responses_header_order_differs():
    header = ["ResponseID", "SomeCustomColumn", "MessageID", "MatchMethod"]
    ws = FakeWs(header=list(header))
    columns = {3: ["MessageID", "abc123"], 4: ["MatchMethod", "Header"]}
    ws.col_values = lambda col: columns.get(col, [])
    connector = _make_sheets_connector_with_responses_ws(ws)
    info = connector.get_logged_message_info()
    assert info == {"abc123": {"row": 2, "match_method": "Header"}}


def test_update_response_match_writes_by_name_when_responses_header_order_differs():
    header = ["ResponseID", "ActionTaken", "SomeCustomColumn", "MatchMethod"]
    ws = FakeWs(header=list(header))
    connector = _make_sheets_connector_with_responses_ws(ws)
    connector.update_response_match(5, "Subject", "logged")
    written = {col: val for _row, col, val in ws.updated_cells}
    assert written[header.index("MatchMethod") + 1] == "Subject"
    assert written[header.index("ActionTaken") + 1] == "logged"


def test_mark_responses_read_writes_by_name_when_responses_header_order_differs():
    header = ["ResponseID", "IsRead", "SomeCustomColumn"]
    ws = FakeWs(header=list(header))
    connector = _make_sheets_connector_with_responses_ws(ws)
    connector.mark_responses_read({"r1": 4})
    written = {col: val for _row, col, val in ws.updated_cells}
    assert written[header.index("IsRead") + 1] == "Yes"
...
def test_call_with_gspread_retries_retries_on_429_rate_limit(monkeypatch):
    ...
    # Jittered, so assert the RANGE rather than an exact value — the exact
    # value is deliberately random so concurrent runners de-synchronise
    # instead of retrying in lockstep (see _quota_retry_delay).
    assert len(sleep_calls) == 1
    assert 7.5 <= sleep_calls[0] <= 22.5  # 15s +/-50%
...
def test_call_with_gspread_retries_429_uses_the_full_longer_schedule(monkeypatch):
    ...
    result = outreach._call_with_gspread_retries(flaky)
    assert result == "finally worked"
    # Full schedule in order, each jittered within +/-50% of its base.
    assert len(sleep_calls) == 3
    for actual, base in zip(sleep_calls, outreach.GSPREAD_QUOTA_RETRY_DELAYS_SECONDS):
        assert base * 0.5 <= actual <= base * 1.5


def test_quota_retry_delay_is_jittered_not_lockstep():
    """The actual reported production issue: several scheduled workflows
    share ONE service account and therefore ONE per-minute Sheets read
    quota, so overlaps are routine. With a FIXED backoff, two colliding
    runners waited the identical interval and retried at the same
    instant — colliding again, then again — turning one unlucky overlap
    into a near-guaranteed failure (the "1 in 5 runs" pattern). Jitter
    lets them de-synchronise."""
    samples = [outreach._quota_retry_delay(15) for _ in range(200)]
    assert len(set(samples)) > 100, "delays must vary, not be fixed"
    assert min(samples) >= 7.5 and max(samples) <= 22.5


def test_quota_retry_delay_never_returns_a_useless_tiny_wait():
    """Jitter must never produce a near-zero delay that would hammer the
    API immediately after a quota rejection."""
    assert all(outreach._quota_retry_delay(b) >= 1.0
                for b in (1, 2, 15, 30, 60, 90) for _ in range(50))


def test_quota_retry_schedule_spans_more_than_one_quota_window():
    """The quota resets per MINUTE, so the schedule must be able to wait
    out more than one full window even at the low end of its jitter."""
    lowest_total = sum(b * 0.5 for b in outreach.GSPREAD_QUOTA_RETRY_DELAYS_SECONDS)
    assert lowest_total >= 60
...
def test_compute_campaign_dashboard_genuine_replies_counts_unique_leads_not_messages():
    """The actual reported production bug: 33 leads, 33 sent, 51
    "replies", a mathematically impossible 154.5% reply rate. Root
    cause — a single lead with several back-and-forth messages in one
    conversation had every message counted separately, inflating the
    total. Reproduced directly: two leads, one with THREE genuine
    response rows logged (a real multi-message conversation), one with
    one — the count must be 2 (unique leads), never 4 (total messages)."""
    ts = "2026-08-20 10:00:00"
    leads = [
        _dash_lead(_row=2, LeadID="L1", Email="chatty@abc.com", IntroSentAt=ts, IntroVariant="A",
                   SenderAccount="sales1", CurrentStage="intro", Status=outreach.STATUS_STOPPED_REPLIED,
                   ReplyStatus="Replied"),
        _dash_lead(_row=3, LeadID="L2", Email="onereply@abc.com", IntroSentAt=ts, IntroVariant="A",
                   SenderAccount="sales1", CurrentStage="intro", Status=outreach.STATUS_STOPPED_REPLIED,
                   ReplyStatus="Replied"),
    ]
    send_log = [{"Status": "sent", "SenderAccount": "sales1"}, {"Status": "sent", "SenderAccount": "sales1"}]
    # THREE genuine response rows for the SAME lead's conversation, plus
    # one for the other lead — 4 rows total, but only 2 unique leads.
    responses = [
        {"Classification": outreach.CLASSIFICATION_GENUINE},
        {"Classification": outreach.CLASSIFICATION_GENUINE},
        {"Classification": outreach.CLASSIFICATION_GENUINE},
    ...
    rows = outreach.compute_campaign_dashboard(campaign_cfg, leads, responses, send_log, [])
    d = _rows_to_dict(rows)

    assert d[("Overview", "Genuine Replies")] == "2"
    assert d[("Overview", "Reply Rate (Replies / Unique Contacted)")] == "100.0%"


def test_compute_campaign_dashboard_genuine_replies_includes_manual_overrides():
    """The second half of what was reported: a lead whose ReplyStatus
    was set by a manual override (set-lead-override), with NO
    corresponding response row at all (a manual override never creates
    one), must still count toward Genuine Replies."""
    ts = "2026-08-20 10:00:00"
    leads = [
        _dash_lead(_row=2, LeadID="L1", Email="manual@abc.com", IntroSentAt=ts, IntroVariant="A",
                   SenderAccount="sales1", CurrentStage="intro", ReplyStatus="Replied"),
    ]
...
def test_compute_campaign_dashboard_reply_rate_never_exceeds_100_percent():
    """The concrete, mathematically-impossible symptom reported (154.5%)
    must be structurally impossible now: every lead can only ever
    contribute at most one to genuine_replies, so the rate can never
    exceed unique_contacted."""
    ts = "2026-08-20 10:00:00"
    leads = [_dash_lead(_row=i, LeadID=f"L{i}", Email=f"l{i}@abc.com", IntroSentAt=ts, IntroVariant="A",
                         SenderAccount="sales1", CurrentStage="intro",
                         Status=outreach.STATUS_STOPPED_REPLIED, ReplyStatus="Replied")
             for i in range(2, 5)]  # 3 leads, all replied
    send_log = [{"Status": "sent", "SenderAccount": "sales1"}] * 3
    # Deliberately MORE response rows than leads, simulating heavy
    # back-and-forth on every single conversation.
    responses = [{"Classification": outreach.CLASSIFICATION_GENUINE} for _ in range(10)]
    campaign_cfg = {"stages": DASH_STAGES}

    rows = outreach.compute_campaign_dashboard(campaign_cfg, leads, responses, send_log, [])
    d = _rows_to_dict(rows)

    assert d[("Overview", "Genuine Replies")] == "3"
    assert d[("Overview", "Reply Rate (Replies / Unique Contacted)")] == "100.0%"
...
# ---------- stale pin survives rotation being toggled OFF (found end-to-end) ----------

def test_resolve_account_stale_pin_cleared_when_rotation_toggled_off():
    """The actual reported production bug: a campaign launched with 5
    rotation accounts, later reduced to 1 — and the natural next step,
    unchecking "Rotate across multiple sender accounts" since there is
    only one account left, silently disabled the ENTIRE stale-pin
    protection. Every lead pinned to one of the 4 removed accounts
    stayed stuck on a sender no longer in this campaign's configuration,
    forever, regardless of how the rotation TOGGLE was set."""
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales3")
    campaign_cfg = {"sending": {"sender_rotation": False, "rotation_accounts": ["sales1"]}}
    kind, value = outreach._resolve_account_for_round(
        lead, campaign_cfg, ROTATION_ACCOUNTS, {}, {}, set())
    assert (kind, value) == ("account", "sales1")


def test_resolve_account_stale_pin_cleared_with_multiple_remaining_accounts():
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales_removed")
    accounts = dict(ROTATION_ACCOUNTS)
    accounts["sales_removed"] = {"address": "old@x.com", "app_password": "p"}
    campaign_cfg = {"sending": {"sender_rotation": False, "rotation_accounts": ["sales1", "sales2"]}}
    kind, value = outreach._resolve_account_for_round(
        lead, campaign_cfg, accounts, {}, {}, set())
    assert kind == "account"
    assert value in ("sales1", "sales2")


def test_resolve_account_cleared_pin_does_not_resurrect_from_original_lead_dict():
    """The specific mechanism that let the bug survive the first
    staleness-check fix: `requested` gets cleared to "" locally, but the
    fallback path used to call resolve_sender_account(lead, ...) with
    the ORIGINAL lead object, which still carried the stale
    SenderAccount and silently re-derived the very account the
    staleness check just rejected."""
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales3")
    campaign_cfg = {"default_sender_account": "sales1",
                     "sending": {"sender_rotation": False, "rotation_accounts": []}}
    kind, value = outreach._resolve_account_for_round(
        lead, campaign_cfg, ROTATION_ACCOUNTS, {}, {}, set())
    assert (kind, value) == ("account", "sales1")


def test_resolve_account_stale_pin_check_consistent_with_actual_resolution():
    """current_valid_senders_for_campaign (used to judge staleness) and
    the actual account-picking logic must agree on what "valid" means —
    otherwise a pin can be correctly judged stale and then STILL fail to
    resolve to anything, because the fallback path doesn't know about
    the same account list the staleness check just consulted."""
    # sales3 genuinely exists in EMAIL_ACCOUNTS_JSON (ROTATION_ACCOUNTS)
    # but is no longer in THIS campaign's rotation_accounts — the pin is
    # stale relative to the campaign, not unknown globally.
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales3")
    campaign_cfg = {"sending": {"sender_rotation": False, "rotation_accounts": ["sales1"]}}
    valid = outreach.current_valid_senders_for_campaign(campaign_cfg, ROTATION_ACCOUNTS)
    kind, value = outreach._resolve_account_for_round(
        lead, campaign_cfg, ROTATION_ACCOUNTS, {}, {}, set())
    assert valid == ["sales1"]
    assert (kind, value) == ("account", "sales1")
...
def test_resolve_account_still_defers_when_the_reassigned_account_is_busy():
    """The concurrent-round defer semantics must survive reassignment —
    a lead reassigned to the one remaining account, when that account
    is already claimed this round, must defer, not error."""
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales3")
    campaign_cfg = {"sending": {"sender_rotation": False, "rotation_accounts": ["sales1"]}}
    kind, value = outreach._resolve_account_for_round(
        lead, campaign_cfg, ROTATION_ACCOUNTS, {}, {}, {"sales1"})
    assert (kind, value) == ("defer", None)


def test_resolve_sender_account_for_send_stale_pin_fixed_identically():
    """The dead-but-tested twin function must never silently regress to
    the version of this bug where a pin to a removed account was
    honored forever — kept in sync even though it currently has zero
    callers in the codebase."""
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales3")
    campaign_cfg = {"sending": {"sender_rotation": False, "rotation_accounts": ["sales1"]}}
    assert outreach.resolve_sender_account_for_send(
        lead, campaign_cfg, ROTATION_ACCOUNTS, {}, {}) == "sales1"
...
def test_validate_campaign_accepts_six_stages_the_actual_reported_case(tmp_path):
    """The actual reported production bug: a campaign with intro plus 5
    follow-ups (6 stages total) was rejected with "5 max" — a leftover
    validation check from before follow-ups were extended from 4 to 10.
    MASTER_COLUMNS and CANONICAL_STAGE_ORDER were both extended at the
    time; this ONE remaining hardcoded "5" in _validate_campaign was
    missed, and had no test coverage at all to catch it."""
    settings_path, campaigns_dir, templates_root = _make_config_fixture(tmp_path, create_templates=False)
    campaign_dir = os.path.join(templates_root, "test_campaign")
    os.makedirs(campaign_dir, exist_ok=True)
    for stage in ["intro", "followup1", "followup2", "followup3", "followup4", "followup5"]:
        with open(os.path.join(campaign_dir, f"{stage}_A.txt"), "w") as f:
            f.write("Subject: Hi\n\nBody")

    cfg = outreach.get_campaign("test_campaign", settings_path=settings_path,
                                 campaigns_dir=campaigns_dir, templates_root=templates_root)
    assert len(cfg["stages"]) == 6
    assert cfg["stages"][-1]["template_prefix"] == "followup5"


def test_validate_campaign_accepts_the_full_eleven_stage_maximum(tmp_path):
    settings_path, campaigns_dir, templates_root = _make_config_fixture(tmp_path, create_templates=False)
    campaign_dir = os.path.join(templates_root, "test_campaign")
    os.makedirs(campaign_dir, exist_ok=True)
    ...
    cfg = outreach.get_campaign("test_campaign", settings_path=settings_path,
                                 campaigns_dir=campaigns_dir, templates_root=templates_root)
    assert len(cfg["stages"]) == len(outreach.CANONICAL_STAGE_ORDER)


def test_validate_campaign_message_names_the_real_current_limit():
    """The error message itself must stay accurate to the real, current
    cap — not a hardcoded number that silently goes stale the same way
    the length check itself just did."""
    fake_cfg = {
        "templates_dir": "x", "sending": {},
        "stages": [{"template_prefix": f"s{i}"} for i in range(len(outreach.CANONICAL_STAGE_ORDER) + 1)],
        "variants": ["A"], "sheet_id": "real_sheet_id",
    }
    with pytest.raises(outreach.ConfigError) as exc_info:
        outreach._validate_campaign("test_campaign", fake_cfg)
    assert str(len(outreach.CANONICAL_STAGE_ORDER)) in str(exc_info.value)
...
def test_most_recent_send_at_finds_a_send_past_followup_four():
    """The actual bug found while auditing this change: the original
    version hardcoded exactly Intro + FollowUp1-4, so for any lead that
    progressed further than that, it reported a stale, too-early "most
    recent send" — the true most recent lives in a FollowUp5+ field it
    never looked at, undermining the exact chronological sanity check
    this function exists to provide."""
    old = "2026-08-01 10:00:00"
    truly_latest = "2026-09-01 10:00:00"
    lead = _most_recent_send_at_lead(IntroSentAt=old, FollowUp4SentAt="2026-08-15 10:00:00",
                                      FollowUp7SentAt=truly_latest)
    result = outreach._most_recent_send_at(lead)
    assert result == outreach._parse_dt(truly_latest)
...
def test_message_to_dict_converts_non_utc_offset_to_utc_not_just_strips_it():
    '''The actual reported production bug, reproduced with the exact
    real numbers: a reply genuinely sent AFTER the most recent outreach
    email, from a sender in US Eastern time (UTC-4), had its offset
    simply discarded rather than applied — silently keeping the
    sender's own local wall-clock time as if it were already UTC. A
    reply timestamped "13:58:30 -0400" (i.e. 17:58:30 UTC) must parse
    to 17:58:30, not 13:58:30 — the ~4 hour gap this bug produced was
    large enough to make a genuinely later reply look like it arrived
    BEFORE the send it was replying to, misclassifying a real
    "Stopped Sequence" reply as "Logged Only (Predates Contact)" and
    never actually stopping the sequence.'''
    msg = MIMEText("body")
    msg["Subject"] = "genuine reply"
    msg["From"] = "george@aol.com"
    msg["Message-ID"] = "<x@mail.gmail.com>"
    msg["Date"] = "Mon, 21 Sep 2026 13:58:30 -0400"
    parsed = outreach._parse_email_message(msg.as_bytes())
    assert parsed["date"] == datetime(2026, 9, 21, 17, 58, 30)
    assert parsed["date"].tzinfo is None  # naive, matching every other timestamp in this system


def test_message_to_dict_utc_offset_unchanged():
    """A message already in UTC (+0000) is a no-op conversion — this is
    the specific case that let the original bug hide in CI, since a
    UTC-based test runner's formatdate(localtime=True) never exercises
    a real non-zero offset at all."""
    msg = MIMEText("body")
    msg["Subject"] = "already utc"
    msg["From"] = "a@b.com"
    msg["Message-ID"] = "<x@mail.gmail.com>"
    msg["Date"] = "Mon, 21 Sep 2026 17:58:30 +0000"
    parsed = outreach._parse_email_message(msg.as_bytes())
    assert parsed["date"] == datetime(2026, 9, 21, 17, 58, 30)


def test_message_to_dict_positive_offset_converts_correctly():
    """A sender EAST of UTC (e.g. IST, +0530) must also convert
    correctly, not just a negative (US) offset."""
    msg = MIMEText("body")
    msg["Subject"] = "ist sender"
    msg["From"] = "a@b.com"
    msg["Message-ID"] = "<x@mail.gmail.com>"
    msg["Date"] = "Mon, 21 Sep 2026 23:28:30 +0530"
    parsed = outreach._parse_email_message(msg.as_bytes())
    assert parsed["date"] == datetime(2026, 9, 21, 17, 58, 30)
...
def test_canonical_stage_order_supports_at_least_ten_followups():
    """The actual reported requirement: at least 10 follow-ups (11
    stages total, intro included). Deliberately a concrete, specific
    assertion — not derived from the list's own length — since the
    whole point is to catch the list ever being silently shrunk back
    below what was actually requested."""
    assert len(outreach.CANONICAL_STAGE_ORDER) >= 11
    assert outreach.CANONICAL_STAGE_ORDER[0] == "intro"
    for n in range(1, 11):
        assert f"followup{n}" in outreach.CANONICAL_STAGE_ORDER


def test_master_columns_has_sent_at_and_variant_for_every_followup_through_ten():
    for n in range(1, 11):
        assert f"FollowUp{n}SentAt" in outreach.MASTER_COLUMNS
        assert f"FollowUp{n}Variant" in outreach.MASTER_COLUMNS


def test_discover_stages_actually_finds_followup_ten_on_disk(tmp_path):
    """End-to-end proof, not just a constant-list check: a real
    templates folder with intro through followup10 must be fully
    discoverable, stage 10 included."""
    d = tmp_path / "templates"
    d.mkdir()
    for stage in outreach.CANONICAL_STAGE_ORDER:
        _write_template(d, f"{stage}_A.txt")
    stages, variants = outreach.discover_stages_and_variants(str(d), {})
    assert stages[-1]["template_prefix"] == "followup10"
    assert len(stages) == 11
    assert variants == ["A"]


def test_discover_every_stage_in_canonical_order_all_variants(tmp_path):
    """Deliberately asserts against len(CANONICAL_STAGE_ORDER) itself,
    not a hardcoded number — this test's whole point is to prove
    discovery scales to however many stages that list holds, so it must
    never itself go stale the way the fixed _most_recent_send_at bug
    did when the list was extended from 5 to 11 entries."""
    d = tmp_path / "templates"
    d.mkdir()
    for stage in outreach.CANONICAL_STAGE_ORDER:
        for v in outreach.ALL_VARIANT_LETTERS:
            _write_template(d, f"{stage}_{v}.txt")
    stages, variants = outreach.discover_stages_and_variants(str(d), {})
    assert len(stages) == len(outreach.CANONICAL_STAGE_ORDER)
    assert [s["template_prefix"] for s in stages] == outreach.CANONICAL_STAGE_ORDER
